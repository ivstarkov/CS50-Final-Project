...
class MainWindow(QDialog):
    ...
    def browse(self):
        ...
        self.directory = QFileDialog.getExistingDirectory(
            self, "Chose Directory", path, QFileDialog.Option.DontUseNativeDialog
        )
        if self.directory:
    ...
    def analyze(self):
        ...
        if self.remove_list:
            self.listWidget_1.addItems(self.remove_list)
            self.listWidget_2.addItems(self.rename_list)
            self.delete_btn.setEnabled(True)
        else:
            self.listWidget.addItem(
                "There are no duplicates to remove in current directory"
            )
